...
describe("TapGestureRecognizer", () => {
  ...
  function createPointerEvent({ type, x, y, identifier }: { type: string; x: number; y: number; identifier: number }) {
  ...
  function sendPointerEventFromEvent({ event, container }: { event: PointerEvent; container: HTMLElement }) {
  ...
  it("should detect a single tap", async () => {
    ...
    const recognizer = createTapRecognizer({ multiTapThreshold })({ container })
      ...
      recognizer.events$.pipe(
        tap({
          subscribe: async () => {
            ...
            sendPointerEvent({ container, identifier: 1, type: "pointerdown", x: 0, y: 0 })
            sendPointerEvent({ container, identifier: 1, type: "pointerup", x: 0, y: 0 })
          },
        }),
  ...
  it("should detect a double tap", async () => {
    const multiTapThreshold = 5
    const waitLongEnough$ = timer(multiTapThreshold * 2)
    const recognizer = createTapRecognizer({ multiTapThreshold, maxTaps: 2 })({ container })
      ...
      recognizer.events$.pipe(
        tap({
          subscribe: async () => {
            ...
            sendPointerEvent({ container, identifier: 1, type: "pointerdown", x: 0, y: 0 })
            sendPointerEvent({ container, identifier: 1, type: "pointerup", x: 0, y: 0 })
            sendPointerEvent({ container, identifier: 2, type: "pointerdown", x: 0, y: 0 })
            sendPointerEvent({ container, identifier: 2, type: "pointerup", x: 0, y: 0 })
          },
        }),
        buffer(waitLongEnough$),
        first(),
  ...
  it("should detect a triple tap", async () => {
    const multiTapThreshold = 5
    const recognizer = createTapRecognizer({ multiTapThreshold, maxTaps: 3 })({ container })
  ...
  it("should not trigger tap if moved too much between down and up", async () => {
    const multiTapThreshold = 5
    const waitLongEnough$ = timer(100)
    const recognizer = createTapRecognizer({ multiTapThreshold, posThreshold: 5 })({ container })
      ...
      recognizer.events$.pipe(
        tap({
          subscribe: async () => {
            ...
            sendPointerEvent({ container, identifier: 1, type: "pointerdown", x: 0, y: 0 })
            sendPointerEvent({ container, identifier: 1, type: "pointerup", x: 50, y: 50 })
          },
        }),
        buffer(waitLongEnough$),
        first(),
  ...
  it("should not detect a tap if the pointer is held down too long", async () => {
    ...
    const waitLongEnough$ = timer(100)
    const recognizer = createTapRecognizer({ multiTapThreshold, posThreshold: 5, maximumPressTime })({ container })
    ...
    expect(values).toMatchObject([{ type: "tap", taps: 1, srcEvent: { pointerId: 1 } }])
  })

  it("should ignore taps when there are simultaneous pointers active", async () => {
    const recognizer = createTapRecognizer({})({ container })
    const waitLongEnough$ = timer(100)
      ...
      recognizer.events$.pipe(
        tap({
          subscribe: () => {
            setTimeout(() => {
              sendPointerEvent({ container, identifier: 1, type: "pointerdown", x: 0, y: 0 })
              sendPointerEvent({ container, identifier: 2, type: "pointerdown", x: 10, y: 10 })
              sendPointerEvent({ container, identifier: 1, type: "pointerup", x: 0, y: 0 })
              sendPointerEvent({ container, identifier: 2, type: "pointerup", x: 10, y: 10 })
            })
          },
        }),
        buffer(waitLongEnough$),
  ...
  it("should ignore one tap if there is more than one tap", async () => {
    const recognizer = createTapRecognizer({ multiTapThreshold: 10, maxTaps: 1 })({ container })
    const waitLongEnough$ = timer(100)
  ...
  it("should ignore taps when a second tap is outside of position threshold", async () => {
    const recognizer = createTapRecognizer({ posThreshold: 5, multiTapThreshold: 10, maxTaps: Infinity })({ container })
    const waitLongEnough$ = timer(100)
