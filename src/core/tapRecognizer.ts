...
const mapToTapEvent = (stream: Observable<[PointerEvent, ...PointerEvent[]]>): Observable<TapEvent> =>
  stream.pipe(
    map((events) => ({
      type: "tap",
      srcEvent: events[0],
...
export const createTapRecognizer = ({
  ...
  return ({
    ...
    const { pointerUp$, pointerLeave$, pointerCancel$, pointerMove$ } = getPointerEvents({
      container,
      afterEventReceived,
    })

    const pointerDown$ = fromPointerDown({ container, afterEventReceived })
    const activePointers$ = trackActivePointers({ container }).pipe(shareReplay(1))
    const hasMoreThanOneActivePointer$ = activePointers$.pipe(filter((pointers) => pointers.length > 1))
    ...
    const bufferPointerDowns = (stream: Observable<PointerEvent>) =>
      stream.pipe(buffer(pointerUp$.pipe(debounceTime(multiTapThreshold))), first())

    const failWith$ = fromFailWith(failWith)

    const tap$ = merge(pointerDown$, activePointers$.pipe(ignoreElements())).pipe(
      onlyOnePointer,
      exhaustMap((initialPointerEvent) => {
        const pointerDowns$ = merge(of(initialPointerEvent), pointerDown$)
        const subsequentPointerEvents$ = merge(pointerUp$, pointerLeave$, pointerCancel$, pointerMove$)
        const pointerDownsBuffered$ = pointerDowns$.pipe(bufferPointerDowns)
        const subsequentPointersOutOfPositionThreshold$ = subsequentPointerEvents$.pipe(
          filter((event) => isOUtsidePosThreshold(initialPointerEvent, event, posThreshold)),
          first(),
        )
        const waitedTooLong$ = activePointers$.pipe(switchMap(() => timer(maximumPressTime)))
